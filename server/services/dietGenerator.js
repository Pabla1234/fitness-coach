...
// ─── 7-Day Meal Plans ─────────────────────────────────────────────────────────
// Each entry: { name, description, portion }   (calories/macros computed dynamically)
...
const PLANS = {
  ...
  // ═══════════════════════════════════════════════════════════
  //  VEGAN  ×  INTERNATIONAL  ×  BUDGET  (alias fallback)
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  //  VEGAN  ×  INDIAN / INTERNATIONAL  ×  PREMIUM
  // ═══════════════════════════════════════════════════════════
  Vegan_Premium: [
    [
...
function resolvePlan(dietType, cuisine, budget) {
  ...
  // Vegan Premium — same plan regardless of cuisine
...
const generateDietPlan = (profile, goal, preferences) => {
  // Support both SQLite snake_case and legacy camelCase field names
  const weight    = profile.current_weight     || profile.currentWeight     || 70;
  const height    = profile.height             || 170;
  const age       = profile.age               || 25;
  const gender    = profile.gender            || 'Male';
  const daysPerWeek = preferences.days_per_week || preferences.daysPerWeek || 4;
  const primaryGoal = goal.primary_goal        || goal.primaryGoal          || 'Athletic Physique';
  const dietType  = preferences.diet_type      || preferences.dietType      || 'Non-Veg';
  ...
  // Calorie distribution across meals (varies by meal count)
  const getDistribution = (mealCount) => {
    if (mealCount === 5) return [0.25, 0.10, 0.30, 0.10, 0.25]; // B, Mid, L, Snack, D
  ...
  // Today's meals (for the simple DietCard view)
